import heapq
import time
global max_depth, hashT, HEURISTIC_SCORE, R, Q, R5, R6, A, B, cacheT, count, count_leaf, count_id, delay_time, total
max_depth = 7
total = 0
delay_time = 0
count_id = 0
...
class node:
    def __init__(self, role=None, move=None, lastCache=None, lastID=None, parent=None, reverse=False):
        ...
        self.cache = self.computeCache(lastCache)
            # self.cache = self.computeCache(lastCache)
        if self.depth > 1:
            if abs(abs(self.cache) - 1000000) < 10000:
                if self.role == 'x': self.cache = float("inf")
                else: self.cache = float("-inf")
