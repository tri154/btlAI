...
host = 'http://127.0.0.1:10504'  # Địa chỉ server trọng tài mặc định
team_id = 1  # team_id mặc định
                    ...
                    for i in range(len(self.board)):
                        for j in range(len(self.board)):
                            if self.board[i][j] != ' ':
                                ...
                                break
                    move = None
                    if empty: move = (len(self.board) / 2, len(self.board) / 2)

                    else: move = TicTacToeAi_truong.calculate_next_move(self.board, 4)
                    print("Move: ", move)
